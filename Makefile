...
# Note that Bash `=~` doesn't allow quoting the regex.
override lib_name_to_name_version_low = $(if $(__cached_lib_name_ver_$1),,$(call var,__cached_lib_name_ver_$1 := $(patsubst lib%,%,$(if $(and $(findstring _,$2)$(findstring -,$2)$(findstring .,$2),$(filter-out 0,$(shell bash -c $(call quote,[[ $2 =~ ^(v|)[0-9.]*$$ ]]))$(.SHELLSTATUS))),$2,$1-$2))))$(__cached_lib_name_ver_$1)
